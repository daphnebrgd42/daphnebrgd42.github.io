...
function getTravelRecommendation(e) {
  ...
  const stationNameEl = stationEl.querySelector(".station-name");
  const stationLocationEl = stationEl.querySelector(".station-location");
  ...
  const showInfo = document.querySelector("#travel-recommendation-info");

  const myHeaders = new Headers();
  myHeaders.append("Content-Type", "application/json");
  myHeaders.append("Authorization", "Bearer app-5SLMHj5xHh2PaINvBOiBhqzv");
  ...
  var result = fetch(
    "https://agent.build-school.com/v1/workflows/run",
    requestOptions
  )
    .then((response) => response.json())
    .then(
      (result) => (showInfo.innerHTML = marked.parse(result.data.outputs.text))
    )
